...
class FilmWork:
    ...
    def save_to_postgres(self, cursor: psycopg2.extensions.connection.cursor):
        cursor.execute(
            ...
            '{None if self.description is None else self.description.replace("'", "`")}',
            '{0.0 if self.rating is None else self.rating}', NOW(), NOW())
...
class Genre:
    ...
    def save_to_postgres(self, cursor: psycopg2.extensions.connection.cursor):
        cursor.execute(f"""
                    ...
                    VALUES ('{self.id}', '{self.name}', '{self.description}', NOW(), NOW())
                    ON CONFLICT (id) DO
...
class PostgresSaver:
    ...
    def save_all_data(self, data):
        self.cursor.execute("""TRUNCATE content.person CASCADE""")
        self.cursor.execute("""TRUNCATE content.genre CASCADE""")
        self.cursor.execute("""TRUNCATE content.film_work CASCADE""")
        self.cursor.execute("""TRUNCATE content.genre_film_work CASCADE""")
        self.cursor.execute("""TRUNCATE content.person_film_work CASCADE""")
        self.pg_conn.commit()
        for i in data:
    ...
    def extract_movies(self):
        with self.conn_context(db_path) as conn:
            ...
            for i in range(0, len(genre_data), 1):
                genre_ = Genre(id=dict(genre_data[i])['id'], name=dict(genre_data[i])['name'],
                               description=dict(genre_data[i])['description'])
                data_.append(genre_)
            for i in range(0, len(person_data), 1):
                person_ = Person(id=dict(person_data[i])['id'], full_name=dict(person_data[i])['full_name'])
                data_.append(person_)
            for i in range(0, len(film_work_data), 1):
                filmwork_ = FilmWork(id=dict(film_work_data[i])['id'], title=dict(film_work_data[i])['title'],
                                     description=dict(film_work_data[i])['description'],
                                     creation_date=dict(film_work_data[i])['creation_date'],
                                     type=dict(film_work_data[i])['type'], rating=dict(film_work_data[i])['rating'])
